...
// TODO: I really need to find a formatter that allows me to make/keep this nice
ParseRule rules[] = {
...
static void declareVariable()
{
    ...
    for (int i = current->localCount - 1; i >= 0; i--)
    {
        ...
        if (identifiersEqual(name, &local->name))
        {
            error("Already a variable with this name is in this scope.");
        }
    }
    addLocal(*name);
}
...
static void forStatement()
{
    ...
    if (!match(TOKEN_RIGHT_PAREN))
    {
        ...
        int incrementStack = currentChunk()->count;
        expression();
        emitByte(OP_POP);
        consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clauses.");

        emitLoop(loopStart);
        loopStart = incrementStack;
        patchJump(bodyJump);
    }
